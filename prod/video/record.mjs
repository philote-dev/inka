...
//
// Drives the clickable prototype (prod/pgrep-prototype.html) with an
...
//
// Run:  node prod/video/record.mjs
// Tune: PACE=1.15 node prod/video/record.mjs   (global speed multiplier)
...
const PROTO = pathToFileURL(path.join(ROOT, "prod/pgrep-prototype.html")).href;
const EXAM = pathToFileURL(path.join(ROOT, "design/assets/reference/exam-dark.png")).href;
const MOBILE = pathToFileURL(path.join(ROOT, "design/assets/reference/mobile-dark.png")).href;
const OUTDIR = path.join(ROOT, "prod/video/raw");
const EXE =
...
(async () => {
    ...
    const stage = (src, mode, c) =>
        page.evaluate(([src, mode, c]) => window.VO.stage(src, mode, c), [src, mode, c || ""]);
