...
-->
<!--
The Library calibration walkthrough, extracted verbatim from the Library page so
it can be one arm of the two-state Library switch (see the card-sets plan §3):
the wheel when calibrated / AI off, this walkthrough while AI is on and
calibration is incomplete. Phase 1 renders the wheel at /pgrep/library and keeps
this component ready; Phase 3 wires the switch and condenses the walk to the nine
blueprint categories. Behaviour is unchanged from the original page.
...
<script lang="ts">
    ...
    // The nine blueprint categories, in blueprint order. Calibration walks one
    // card per category (card-sets plan §1.7), so the walkthrough and the gate
    // (one authored card per category) line up exactly.
    const TOPICS: { tag: string; label: string }[] = [
        { tag: "topic::mechanics", label: "Classical Mechanics" },
        { tag: "topic::electromagnetism", label: "Electromagnetism" },
        { tag: "topic::quantum", label: "Quantum Mechanics" },
        {
    ...
    // Fired after each card is authored, so the Library can re-check calibration
    // coverage and switch to the wheel once every category is covered.
    export let onAuthored: (() => void) | undefined = undefined;
    ...
    async function generate(): Promise<void> {
        ...
        try {
            ...
            // A card landed (the seed always enters, AI on or off): let the
            // Library re-check calibration coverage and switch to the wheel once
