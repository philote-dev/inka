# Parallel Track (what to do while the agents build)

**What this is.** The **when** of your non-code work, sequenced against the build so you are never the thing blocking a layer. The **what and how** already live in `[setup-content-deps.md](setup-content-deps.md)` (sourcing tiers, RAG assets, tools, accounts, cost). The **build order** lives in `[build-plan.md](build-plan.md)`. This doc ties your work to that order. It does not repeat the detail, it points at it.
...
- **Stand up the AI toolchain.** LLM API key, local embeddings running (`sentence-transformers` or `bge`), a local vector store (`sqlite-vec` or FAISS), and SymPy for computational checks. Detail: `setup-content-deps.md` §3.2.
...
- `[setup-content-deps.md](setup-content-deps.md)` · the full detail on sourcing, RAG assets, tools, accounts, and cost.
