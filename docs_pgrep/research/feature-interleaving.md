...
> **Persona refinement (locked, applies to all features):** users are **post-undergraduate** students who have already been taught this material and want to ace the test. **No true novices / no cold-starts** in core scope. Lead with the three pillars in order — **retrieval (cards) → practice questions → practice tests** — and defer _teaching fallbacks_ (curated content/Learn surface, learn-by-explaining / self-explanation) to after the core works.
...
- **Knob decisions (resolved):**
  - **K (anti-blocking): fixed.** A small constant (2–3) ships as-is; it is a guardrail, not an adaptive knob.
  - **Session budget: fixed** for now, one default size, revisited later.
  - **Cards/Problems balance: dropped.** There is no card↔problem mixing, so the selector never splits a session across doors. The user enters one door. What remains is the **per-door budget** (each tuned on its own), rough starting points: Cards ≈ 20–40 reviews, Problems ≈ 5–10 problems (the ladder + self-explanation make problems long). Defaults to calibrate.
  - **Difficulty band: ship the literature default.** Use 60–85% predicted success as a soft preference (not a hard filter); it only reorders already-due items, so it cannot break anything. Predicted success for a brand-new problem uses the forced-generation AI difficulty rating (`D0 = 1 + 9·difficulty`) as the proxy until real attempts accrue; cards get it free from FSRS R.
  - **Value-vs-band weighting: fixed for core, then readiness/time-aware (post-core).** The worth↔band tradeoff barely matters early and matters most near the test. Core ships a fixed weighting. The v2 enhancement anneals it along a dial: **learning mode** (weight the difficulty band, build transfer) when the test is far and coverage is thin, toward **exam mode** (weight worth = blueprint% × weakness, triage for score) as the test nears. The dial is driven by **time to test** (Settings date) and **coverage/readiness** (from the same attempt log that feeds the Performance + Readiness models). This closes a feedback loop: study what would move the projected score most. Honesty guard: the selector steers study only, never the held-out measurement, so the loop never grades its own homework.
- **Sequencing note (by dependency, not calendar):** the readiness/time-aware weighting depends on the Performance and Readiness models existing, so it comes after the memory-only core works. The graded selector ships with a fixed weighting first; the annealing is a strong follow-on, not part of the hero change's minimum.
- **Open (remaining):** exact numeric defaults for the per-door budgets, band edges, and the weighting schedule (all defaulted, then simulation-tuned); the tiebreak when Worth and the difficulty-band disagree.
...
- **Exam mode:** zero help — the _measuring_ instrument (readiness + pacing).
- **The session:** the _training_ instrument; recommended path keeps topics mixed inside each door.
...
| Concept induction | Kornell & Bjork 2008              | spacing/interleaving beats massing; learners _misjudge_ it (metacognitive illusion) |

**Mechanism (Rohrer 2014):** interleaving forces you to _choose the strategy from the problem itself_ → trains discrimination + problem→strategy pairing = exactly the PGRE skill.
**Cost (Taylor & Rohrer 2010; Soderstrom & Bjork 2015):** interleaving _depresses in-session performance_ but _raises learning/transfer_ — so we must not optimize session accuracy.

---

## Literature menu for the scheduler (math + results)

The mixing policy sits in **two layers**: _when_ an item is due (scheduling) and _which_ due item comes next (selection). FSRS already handles Layer A in the Anki engine we're forking; Layer B is what we add.
...
| **DKT / AKT** (Piech 2015; Ghosh 2020)                       | RNN / attention over interaction sequence                                                                   | mastery / next-correct prob       | DKT beats BKT, but _extended BKT matches DKT_; AKT adds interpretability; heavier/data-hungry                                |
...
- **The loop (cohort synthesis):** attempt → feedback → self-explain → retry later → transfer to a _related but different_ problem. Practice tests stay separate (timed, no hints) and convert missed/lucky-correct items into spaced follow-ups.
...
- **Verified — anti-blocking does NOT fight FSRS fuzz.** Fuzz is _interval-level only_: `with_review_fuzz(interval, …) → scheduled_days` (`rslib/src/scheduler/states/fuzz.rs`), applied at every interval site (`review.rs`, `learning.rs`, `relearning.rs`, `memory_state.rs`). It sets _due dates_ (± days) = _which_ cards are in today's pile; our anti-blocking orders _within_ that pile. Different layers → additive, no conflict; Anki has no within-session topic ordering today, so nothing to combat. Edge: fuzz + load-balancer can skew a day's pile toward some topics (less to interleave on skewed days).
- **Verified — ahead-of-schedule practice won't corrupt FSRS, and performance still counts.** Memory (FSRS) and Performance (ours) are separate. (1) FSRS already models elapsed time — `timing.next_day_at.elapsed_days_since(*last_review)` (`rslib/src/scheduler/fsrs/memory_state.rs`) — so an early review is just a low-elapsed review, not corruption. (2) The engine already has practice-without-rescheduling — filtered decks (`rslib/src/scheduler/filtered/`, `states/rescheduling_filter.rs`, test `set_due_date_is_filtered`). **Design:** the "nothing due, practice anyway" path routes through filtered/no-reschedule → the attempt logs to Performance + calibration (it _does_ count) but does not drive FSRS rescheduling (keeps low-information early reviews out of stability).

## Working design — Layer B: expected-score-impact selector (direction approved)

Among **due** items (FSRS Layer A untouched):

1. **Points-at-stake value:** score = `blueprint% × topic weakness`, weakness = `1 − mean topic R`. _(Spec's own construct; ITS utility tradition.)_
2. **Desirable-difficulty band:** prefer items with predicted success ≈ **60–85%** (from R and D; no new model). _(Bjork desirable difficulties; Wilson et al. 2019 "Eighty-Five Percent Rule" — derived for binary-classifier training, apply with stated assumptions; Metcalfe region of proximal learning.)_
3. **Anti-blocking:** max K consecutive same-topic items. _(Rohrer/Taylor interleaving.)_
4. **Constants** (band edges, K, value-vs-band weighting) tuned against **FSRS-simulated synthetic students** _(MEMORIZE/SSP-MMC methodology)_, then **ablation-tested on real use** (full / interleaving-off / plain Anki).

Novel as a _composition_; every ingredient literature-backed. Doubles as the graded Rust change (Phase 3 locks implementation).
...
| Spec perf targets (50k cards, p95 latencies) | The MacBook _is_ the reference machine                                                                                                                                      | Fine                                            |
