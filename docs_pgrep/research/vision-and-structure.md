...
- **Build performance** — exam-style problems that test *application* to novel questions; help that pushes the learner's reasoning before revealing answers.
...
**Architecture stance:** pgrep is a **product built on the shared, modified Anki Rust engine.** "Reuse Anki" means reuse the *engine*, not the *interface*.
...
- **4. Content:** Author & Generate (write a seed item → AI conforms to your style; gold-set check) + browse/manage cards & problems. *(Learn/remediation surface deferred — teaching fallback.)*
- **5. Settings:** AI on/off, sync, target retention, test date.

*Cross-cutting under every surface:* the shared engine (FSRS + our scheduler change), the review/attempt log (feeds all three scores + calibration), sync (desk ↔ phone).
...
- One real **Rust engine change** end-to-end: diff + 3 Rust unit tests + 1 Python-side test. *(Leading candidate: interleaving scheduler — doubles as Sunday's ablation feature.)*
