...
primary checkout on `main` (so agents can work on `main` undisturbed); never do
feature work directly on `main`, or by checking a feature branch out in the
primary checkout.

- Start a feature: `git worktree add .worktrees/<feature> -b <feature> main`
- Work inside `.worktrees/<feature>/` (the `.worktrees/` dir is gitignored).
- Finish: merge / open a PR, then `git worktree remove .worktrees/<feature>` and
  delete the branch.
