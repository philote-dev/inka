...
export function drawManifold(canvas, opts = {}) {
    ...
    const clip = (a, b) => {
        ...
        for (let k = 0; k < 9; k++) {
            const mid = (lo + hi) / 2;
            if (visAt(a.x + (b.x - a.x) * mid, a.y + (b.y - a.y) * mid)) { lo = mid; }
            else { hi = mid; }
        }
...
// 2D top-down contour map (marching squares over the same surface).
// Small-screen default and zero-new-dep fallback (d3-contour in production).
// opts = { W, H, S, dpr = 2, glow = 0.8, grid = 80, levels, indexEvery, surface }
// indexEvery: every Nth level draws heavier, topo index-contour style.
// Returns { peaks, gaps } — projected anchors in canvas px so callers can
// place summit markers, topic labels and gap callouts as HTML/SVG overlay.
export function drawContour(canvas, opts = {}) {
    ...
    for (let i = 0; i < n; i++) {
        for (let j = 0; j < n; j++) {
            ...
            for (let li = 0; li < levels.length; li++) {
                ...
                const alpha = Math.min(0.95, glow * (0.35 + 0.12 * li) * (isIdx ? 1.15 : 1));
                ctx.strokeStyle = "rgba(" + c[0] + "," + c[1] + "," + c[2] + "," + alpha.toFixed(3) + ")";
    ...
    // Projected anchors (canvas px) for the label overlay.
    return {
        peaks: surface.bumps.map(b => ({ x: px(b.x), y: py(b.y), h: b.h })),
        gaps: surface.holes.map(h => ({ x: px(h.x), y: py(h.y), rx: h.rx * S, ry: h.ry * S, rot: h.rot || 0 })),
    };
}
