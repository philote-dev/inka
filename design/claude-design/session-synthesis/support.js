...
(() => {
  ...
  `;
    ...
    var INLINE_TEXT_TAGS = new Set(
        "a abbr b bdi bdo br cite code del dfn em i ins kbd mark q s samp small span strike strong sub sup u var wbr"
            .split(
                " ",
            ),
    );
    ...
    function collectProps(node, kind, host) {
        ...
        for (const { name, value } of [...node.attributes]) {
            ...
            if (kind !== "dom") {
                if (
                    key.includes("-") && !(kind === "x-import" && (key.startsWith("aria-") || key.startsWith("data-")))
                ) {
    ...
    function walkXImport(el, host) {
        ...
        const fromRaw = el.getAttribute("from")
            || (el.getAttribute("component-from-global-scope")
                ? ""
                : el.getAttribute("src") || el.getAttribute("import") || "");
    ...
    function walkElement(el, host) {
        ...
        const tplId = el.getAttribute("data-dc-tpl");
        const inlineOnly = el.childNodes.length > 0 && !NEVER_CONTENT_KEYED.has(realTag)
    ...
    function createComponentFactory(registry, ensureFetched) {
        ...
        class StreamableComponent extends React.Component {
            ...
            __reconcileLogic() {
                ...
                if (
                    Next === Cur || !Next && Cur === StreamableLogic
                    || Next === this.__failedLogic && r.ver === this.__failedVer
            ...
            render() {
                const r = registry.get(this.__name);
                const cls = "sc-host" + (r.htmlStreaming ? " sc-streaming-html" : "")
                    + (r.jsStreaming ? " sc-streaming-js" : "");
                const hintStyle = r.htmlStreaming ? hintToMin(this.props.__hintSize) : void 0;
                const hostStyle = this.props.__hostStyle || hintStyle
                ...
                try {
                    ...
                    console.error(e);
                    renderErr = this.__name + ".renderVals(): "
    ...
    function createExternalModules(onResolved) {
        ...
        function resolve2(url, name) {
            ...
            const C = mod && mod[name] || globals && globals[name] || typeof window !== "undefined" && window[name]
                || mod && mod.default;
            ...
            if (!reportedMissing.has(key)) {
                ...
                if (isCE && !customElements.get(name)) {
                    console.warn(
                        ...
                        JSON.stringify(name),
                        "is registered and window." + name + " is not a function \u2014 rendering <" + name
    ...
    function createHelmetManager(doc, isStreaming) {
        ...
        try {
            ...
            appTheme = ds === "dark" || ds === "light"
                ? ds
                : new URLSearchParams(doc.defaultView?.location.search ?? "").get(
                        "theme",
                    ) === "dark"
                ? "dark"
                : "light";
            ...
            return (_vals, ctx) => {
                ...
                for (let i = 0; i < raw.length; i++) {
                    ...
                    if (tag === "SCRIPT") {
                        ...
                        if (mayBePartial) { continue; }
                        const key = tag + "|"
                            + (child.getAttribute("href") || child.getAttribute("src") || child.outerHTML);
                        if (mounted.has(key)) { continue; }
                        mounted.add(key);
    ...
    function createPseudoSheet(doc) {
        ...
        return (pseudo, css) => {
            ...
            const sel = pseudo === "before" || pseudo === "after"
                ? "." + cls + "::" + pseudo
                : "." + cls + ":" + pseudo;
